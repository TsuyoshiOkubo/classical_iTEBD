...
def main():
    ...
    if append_flag:
        file_output = open("Ising_J1J2_output"+tag+".dat","a")
    else:
        file_output = open("Ising_J1J2_output"+tag+".dat","w")
    ...
    Tn_init = (np.random.rand(4)-0.5).reshape(1,4,1)
    lam_init = np.ones(1)
    ...
    while ( delta_lam > epsilon and itr < max_itr):        
        ...
        Tn_D,eig_L_UD = Normalize_two_states(Tn_new,Tn_new.reshape(len(lam_new),2,2,len(lam_new)).transpose(3,2,1,0).reshape(len(lam_new),4,len(lam_new)),lam_new,lam_new)        
        ...
        print(repr(itr)+" "  +repr(-0.5*T*np.log(np.real(eig_L[0] * factor)))+ " " +repr(np.real(ene))+" " +repr(np.real(m))+" " +repr(np.real(mx))+" " +repr(np.real(my))+" "+repr(delta_lam)+ " "+ repr(truncation_error))
#        print(repr(itr)+" "  +repr(-0.5*T*np.log(np.real(eig_L[0] * factor)))+ " " +repr(np.real(ene))+" " +repr(np.real(m))+" " +repr(np.real(mx))+" " +repr(np.real(my))+" "+repr(delta_lam)+ " "+ repr(truncation_error)+" " +repr(eig_R)+" " +repr(eig_L)+ " " +repr(np.dot(vec_R.reshape(-1),vec_L.reshape(-1)))+ " "+repr(np.dot(vec_R.reshape(-1),vec_L.reshape(vec_L.shape[0],2,2,vec_L.shape[0]).transpose(3,2,1,0).reshape(-1))))
    ...
    print(repr(T)+" "  +repr(J2)+ " " +repr(-0.5*T*np.log(np.real(eig_L[0] * factor)))+ " " +repr(np.real(ene))+" " +repr(np.real(m))+" " +repr(np.real(mx))+" " +repr(np.real(my))+" "+repr(itr))

    print("## calculate correlation length (along J2 direction)")
    ## calculate correlation length
    xi = Calc_correlation_length(Tn,Tn_D,lam,lam,1)
    print(repr(T)+" "+ repr(xi[0]*np.sqrt(2)))
    
    file_output.write(repr(T)+" "  +repr(J2)+ " " +repr(-0.5*T*np.log(np.real(eig_L[0] * factor)))+ " " +repr(np.real(ene))+" " +repr(np.real(m))+" " +repr(np.real(mx))+" " +repr(np.real(my))+" "+repr(itr)+" " + repr(xi[0]*np.sqrt(2))+ "\n")
